...
static BseErrorType
load_wave (const gchar *file_name,
	   BseSample  **sample_p)
{
  DataHeader data_header;
  WavHeader wav_header;
  ...
  bdata = bse_object_new (BSE_TYPE_BIN_DATA, NULL);
  bse_bin_data_init_values (bdata, sizeof (BseSampleValue), data_header.data_length / data_width);
  error = read_data (fd, &fmt_header, bdata->n_values, (BseSampleValue*) bdata->values, data_header.data_length);
  close (fd);
  if (error)
    {
      bse_object_unref (BSE_OBJECT (bdata));
      return error;
    }
  
  *sample_p = bse_object_new (BSE_TYPE_SAMPLE,
			      "n_tracks", fmt_header.n_channels,
			      "recording_frequency", fmt_header.sample_freq,
			      NULL);
  bse_sample_set_munk (*sample_p,
		       BSE_KAMMER_NOTE, BSE_KAMMER_NOTE,
		       0, 0,
		       bdata);
  bse_object_unref (BSE_OBJECT (bdata));
           ...
           guint           n_bytes)
{
  ...
  g_assert (n_bytes == (header->bit_per_sample + 7) / 8 * n_values);
  
  /* we convert to 16bits, assuming sizeof (BseSampleValue) == 16 */
  
  if (header->bit_per_sample == 8)
    {
      guint8 *bytes = g_new (guint8, n_bytes);
      
      if (read (fd, bytes, n_bytes) != n_bytes)
	{
	  g_free (bytes);
	  return BSE_ERROR_IO;
	}
	...
	{
	  BseSampleValue value = bytes[i];
	  
#ifdef	INT_SAMPLES
	  values[i] = value << 8;
#else
	  values[i] = value * 256.0;
#endif
	}
      g_free (bytes);
    }
  else if (header->bit_per_sample == 12 || header->bit_per_sample == 16)
    {
      Word *words = g_new (Word, n_bytes / 2);
      
      if (read (fd, words, n_bytes) != n_bytes)
	{
	  g_free (words);
	  return BSE_ERROR_FILE_TOO_SHORT;
	}
      if (header->bit_per_sample == 12)
	for (i = 0; i < n_values; i++)
	  {
	    BseSampleValue value = WORD_FROM_LE (words[i]);
...
{
  gfloat values[4096 * 8];
  GslLong o, l = 0;

  o = G_N_ELEMENTS (values) / 2;
  l = read (0, values + o, G_N_ELEMENTS (values) * 2);
  l /= 2;
  if (l)
    {
      gsl_conv_to_float (GSL_WAVE_FORMAT_SIGNED_16, G_LITTLE_ENDIAN, values + o, values, l);
    }
}
