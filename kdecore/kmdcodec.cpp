...
/******************************** KSHA1 ********************************/
// most of this code is copied from KMD5; however, i haven't been able
// to simply subclass that class because SHA1 uses a bigger buffer
// than MD5.  I know I should have made some generic subclass, but
// I don't think that would be worth the trouble, since there are only
// two hash algorithms here...
KSHA1::KSHA1()
{
  init();
}
...
void KSHA1::update(const unsigned char* in, int len)
{
    ...
    // add in_length to the internal length (m_count)
    if (  (m_count[0] += (in_length << 3))<(in_length << 3) )
      m_count[1]++;
    m_count[1] += (in_length >> 29);

    // space still left in the buffer
    buffer_space = 64 - buffer_index;

    if (in_length >= buffer_space)
    {
      // fill the buffer and transform it
      memcpy (m_buffer + buffer_index, in, buffer_space);
      transform (m_buffer);
      
      // transform everything that's big enough, the rest will be put
      // into the buffer (cf. ultra)
      for (in_index = buffer_space; in_index + 63 < in_length;
	   in_index += 64)
	transform (reinterpret_cast<const unsigned char*>(in+in_index));
...
void KSHA1::finalize ()
{
  ...
  static unsigned char PADDING[64]=
    ...
    // Store state in digest
    //encode (m_digest, m_state, 20);
    memcpy( m_digest, m_state, 20 );
...
QCString KSHA1::hexDigest()
{
  ...
  sprintf(s.data(), "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
	  ...
	  m_digest[14], m_digest[15], m_digest[16], m_digest[17],
	  m_digest[18], m_digest[19]);
...
void KSHA1::transform( const unsigned char block[64] )
{
  Q_UINT32 a = m_state[0],
    b = m_state[1],
    c = m_state[2],
    d = m_state[3],
    e = m_state[4],
  ...
  Q_UINT32 temp;
  for (int t = 0; t != 80; ++t) {
    temp = rotate_left(a, 5) + f(t,b,c,d) + e + w[t] + K(t);
  ...
  m_state[4] = m_state[4] + e;

  a = 0; b = 0; c = 0; d = 0; e = 0;
  memset ( static_cast<void *>(w), 0, sizeof(*w) );
}

void KSHA1::encode ( unsigned char* output, Q_UINT32 *in, Q_UINT32 len )
{
#if !defined(WORDS_BIGENDIAN)
    memcpy(output, in, len);
...
// Decodes in (Q_UINT8) into output (Q_UINT32). Assumes len is a
// multiple of 4.
void KSHA1::decode (Q_UINT32 *output, const unsigned char* in, Q_UINT32 len)
{
#if !defined(WORDS_BIGENDIAN)
    memcpy(output, in, len);
