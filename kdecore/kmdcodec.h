...
// C++ implementation of the SHA1 hash algorithm
/*
 * This class implements the SHA1 hash algorithm (rfc 3174 "US Secure
 * Hash Algorithm.  It works like KMD5.
 * If you have the choice which algorithm to use, use MD5, since that
 * is a more safe algorithm.  This code is meant for protocols which
 * don't give you the choice, like Jabber.
 * Note that I haven't thoroughly tested this code yet, so don't rely
 * on it.  I do think however that it's bugfree...
 *
 * @see KMD5
...
public:
   ...
   * @overload
   *
   * @param input  message to be added to the digest (QCString).
   */
  void update(const QCString& in );
   ...
   * Calling this function will reset the calculated message digest.
   * Use this method to perform another message digest calculation
   * without recreating the KSHA1 object.
   */
  void reset();
   ...
   * copy of the digest once you obtain it.
   *
   * @param bin an array of 20 characters ( char[20] )
   */
...
private:
  ...
  void init();
  // to prevent problems with bigendian and little-endian architectures
  void encode( unsigned char* output, Q_UINT32 *in, Q_UINT32 len );
  void decode( Q_UINT32 *output, const unsigned char* in, Q_UINT32 len );

  inline Q_UINT32 rotate_left( Q_UINT32 x, Q_UINT32 n );
  // some stuff used in the SHA1 algorithm
  // constants:
  inline Q_UINT32 K(int x)
  {
    // these come straight from the rfc, do _not_ touch...
    if (x < 20) return 0x5A827999;
    if (x < 40) return 0x6ED9EBA1;
    if (x < 60) return 0x8F1BBCDC;
    return 0xCA62C1D6;
  };

  inline Q_UINT32 f(int t, Q_UINT32 B, Q_UINT32 C, Q_UINT32 D)
  {
