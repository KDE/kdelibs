...
public:
    ...
    void parseGradientStops(gradient_rendering_properties *props, QDomElement element)
    {
        ...
        for(QDomNode node = element.firstChild(); !node.isNull(); node = node.nextSibling())
        {
            ...
            quint32 qStopOpacity = m_engine->painter()->parseOpacity(parseOpacity);
    ...
    void parseCommonAttributes(QDomNode &node)
    {
        ...
        QList<QDomNamedNodeMap>::const_iterator it = applyList.begin();
        QList<QDomNamedNodeMap>::const_iterator end = applyList.end();
        ...
        for(int i = 0; i < attr.count(); i++)
        {
            ...
            QString nodeName = current.nodeName().toLower();
            QString nodeValue = current.nodeValue();

            if(nodeName == "transform")
                parseTransform(nodeValue);
            else if(nodeName == "style")
                parseStyle(nodeValue);
            else
                parsePA(nodeName, nodeValue);
        }
    }
        ...
        else if(element.tagName() == "image")
        {
            ...
            if(href.startsWith("data:"))
            {
                ...
                */
                ...
                {
                    QImage show = image.scaled((int) w, (int) h, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    ...
    void parseStyle(const QString &style)
    {
        ...
        QStringList::const_iterator it = substyles.begin();
        QStringList::const_iterator end = substyles.end();

        for(; it != end; ++it)
        {
            QStringList substyle = (*it).split(':', QString::SkipEmptyParts);
            QString command = substyle[0], params = substyle[1];
    ...
    void parsePA(const QString &command, const QString &value)
    {
        ...
        else if(command == "a:adobe-blending-mode" || command == "adobe-blending-mode")
        {
            QString compare = value.toLower();

            if(compare == "screen")
                m_engine->painter()->setMode(1);
            else if(compare == "multiply")
                m_engine->painter()->setMode(2);
            else if(compare == "overlay")
                m_engine->painter()->setMode(3);
            else if(compare == "darken")
                m_engine->painter()->setMode(4);
            else if(compare == "lighten")
                m_engine->painter()->setMode(5);
            else if(compare == "hardlight")
                m_engine->painter()->setMode(13);
            else if(compare == "softlight")
                m_engine->painter()->setMode(14);
            else if(compare == "colordodge")
                m_engine->painter()->setMode(15);
            else if(compare == "colorburn")
                m_engine->painter()->setMode(16);
            else if(compare == "exclusion")
                m_engine->painter()->setMode(17);
            else if(compare == "difference")
                m_engine->painter()->setMode(18);
        }
    }
...
bool KSVGIconEngine::load(int width, int height, const QString &path)
{
    ...
    {
        gzFile svgz = gzopen(path.toLatin1(), "ro");
        if(!svgz)
            return false;
    ...
    {
        QStringList points = rootElement.attribute("viewBox").trimmed().split(' ');
...
KSVGIconPainter *KSVGIconEngine::painter() const
{
    return d->painter;
}

QImage *KSVGIconEngine::image() const
{
    return d->painter->image();
}

double KSVGIconEngine::width() const
{
    return d->width;
}

double KSVGIconEngine::height() const
{
    return d->height;
}
