...
int KDateTime::utcOffset() const
{
    ...
    {
        case TimeZone:
            return d->spec.timeZone()->offsetAtZoneTime(d->dt());
        default:
    ...
    for (int i = 0, end = format.length();  i < end;  ++i)
    {
        ...
        else if (zone != TZNone)
        {
            ...
            switch (d->spec.type())
            {
                ...
                case OffsetFromUTC:
                    offset = (d->spec.type() == TimeZone) ? d->spec.timeZone()->offsetAtZoneTime(d->dt())
...
QString KDateTime::toString(TimeFormat format) const
{
    ...
    int offset = 0;
    const char *tzcolon = "";
    const KTimeZone *tz = (d->spec.type() == TimeZone) ? d->spec.timeZone() : 0;
    switch (format)
    {
        case RFCDateDay:
            result += shortDay[d->date().dayOfWeek() - 1];
    ...
    if (d->spec.type() == OffsetFromUTC  ||  tz)
    {
