...
public:
     ...
     * @see #multipleMatches
     */
    bool hasMultipleMatches() const { return hasMultipleMatches(false); }

    /**
     * Same as above except when the flag @p shellmode is set
     * it returns true if and only if there are multiple matches
     * in shell completion mode and the same string was sent
     * more than once to @ref makeCompletion
     */
     // BC: Merge with the above function in 3.0
    bool hasMultipleMatches( bool shellmode ) const;
...
public:
    ...
    /**
     * Sets the completion object to be used.
     *
    ...
    /**
     * Enable/disable the object from emitting text completion
     * and rotation signals.
     *
     * When this function is called with @p enable set to @p false,
     * the completion and rotation signals will not to be emitted.
     * However, unlike setting the completion object to @p NULL
     * using @ref setCompletionObject, this does not interfere with
     * the state of the current completion object, if one has already
     * been created or set.  Simply put it is not deleted. Instead the
     * signals are temporarily disabled.
     *
     * <u>NOTE:</u> If @ref setCompletionObject is invoked after
     * calling this function to disable the emition of these signals,
     * the flag will be reverted back since by emittion occurs by
     * default when a completion object is created by calling either
     * @p completionObject or @p setCompletionObject.  Coincedentally,
     * disabling these signals using this funtion, does not stop the
     * default behavior of the object's ability to handle them internally.
     * If you to change the object's ability to internally handle these
     * signals, see @ref setHandleSignals.
     *
    ...
    */
    virtual void setCompletedText( const QString& txt ) = 0;
