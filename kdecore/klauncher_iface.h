...
#ifndef KLAUNCHER_IFACE_H_295071151665934
#define KLAUNCHER_IFACE_H_295071151665934
...
public Q_SLOTS: // METHODS
    ...
    {
        QList<QVariant> argumentList;
        argumentList << qVariantFromValue(app) << qVariantFromValue(args) << qVariantFromValue(env);
        ...
        if (reply.type() == QDBusMessage::ReplyMessage && reply.count() == 4) {
            dbusServiceName = qdbus_cast<QString>(reply.at(1));
            error = qdbus_cast<QString>(reply.at(2));
            pid = qdbus_cast<int>(reply.at(3));
        }
        return reply;
    }
    ...
    {
        QList<QVariant> argumentList;
        argumentList << qVariantFromValue(app) << qVariantFromValue(args);
        QDBusMessage reply = callWithArgumentList(QDBus::Block, QLatin1String("kdeinit_exec"), argumentList);
        if (reply.type() == QDBusMessage::ReplyMessage && reply.count() == 4) {
            dbusServiceName = qdbus_cast<QString>(reply.at(1));
            error = qdbus_cast<QString>(reply.at(2));
    ...
    inline QDBusReply<int> requestSlave(const QString &protocol, const QString &host, const QString &app_socket, QString &error)
    {
        ...
        if (reply.type() == QDBusMessage::ReplyMessage && reply.count() == 2) {
            error = qdbus_cast<QString>(reply.at(1));
        }
        return reply;
    }
    ...
    {
        QList<QVariant> argumentList;
        argumentList << qVariantFromValue(serviceName) << qVariantFromValue(urls);
        QDBusMessage reply = callWithArgumentList(QDBus::Block, QLatin1String("start_service_by_desktop_name"), argumentList);
        if (reply.type() == QDBusMessage::ReplyMessage && reply.count() == 4) {
            dbusServiceName = qdbus_cast<QString>(reply.at(1));
            error = qdbus_cast<QString>(reply.at(2));
