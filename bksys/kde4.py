...
def generate(env):
	...
	def kcfg_buildit(target, source, env):
		# TODO: why does target[0].dir return under unix only builddir and under windows builddir + subdir
	...
	def kdeinstallicon(lenv, icname='*', path='./', restype='KDEICONS', subdir=''):
		KDEicon(lenv, icname, path, restype, subdir)

	def kdeInstallAuto( lenv, programname=None ):
		""" this function automatically installs kde-specific files, to keep the SConscript files simpler
		...
		if not env['_INSTALL_']:
			return;
		print "this autoinstalls kde files"
		# try to get dir
		if not programname:
			mydir=SCons.Node.FS.default_fs.Dir('.').path
			print "kdeauto dir: %s" % mydir
			sep = SCons.Util.rightmost_separator( mydir, '/' )
			if sep == -1:
				progname = mydir
			else:
				progname = mydir[sep+1:]
		else:
			progname = programname
		print "programname: %s" % progname

		mydir=SCons.Node.FS.default_fs.Dir('.').path
		import os
		lenv.kdesktopfile( '*.desktop' )

		# FIXME make checking for files in source dir work
		#if os.path.isfile( mydir + os.sep + progname + 'ui.rc' ):
		lenv.kdeappdata( progname + 'ui.rc*', progname )
		#if os.path.isfile( mydir + os.sep + 'eventsrc' ):
		lenv.kdeappdata( 'eventsrc*', progname )
		#if os.path.isfile( mydir + os.sep + progname + '.kcfg' ):
		lenv.kdekcfg( progname + '.kcfg*' )
		lenv.kdeicon( progname )

	def kdeinstallinternal( lenv, files, subdir, type ):
		if not env['_INSTALL_']: return
		print "kdeinstallinternal files: %s" % files
		basedir=lenv.getInstDirForResType( type )
		import string
		if string.find( files, '*' ) == -1:
			# no glob char in files
			lenv.bksys_install( env.join( basedir, subdir ), files )
		else:
			# try to apply globbing to the files string
			sourcedir=SCons.Node.FS.default_fs.Dir('.').srcnode()
			
			#import os
			#print "cwd in kde4.py: %s" % os.getcwd()
			#print "sourcedir: %s" % sourcedir
			import glob
			expandedfiles = []
			for i in files.split():
				tmpfiles = glob.glob( str(sourcedir) + '/' + i )
				for tmpfile in tmpfiles:
					expandedfiles.append( tmpfile )
			#print "expandedfiles: %s" % expandedfiles
			#for i in expandedfiles:
			#	print "expanded file: %s" % i
			lenv.bksys_install( env.join( basedir, subdir ), expandedfiles )

	
	def kdeheaders(lenv, files, subdir=''): kdeinstallinternal(lenv, files, subdir, 'KDEINCLUDE' )
	def kdeappdata(lenv, files, subdir=''): kdeinstallinternal(lenv, files, subdir, 'KDEAPPS' )
	def kdedata   (lenv, files, subdir=''): kdeinstallinternal(lenv, files, subdir, 'KDEDATA' )
	def kdekcfg   (lenv, files, subdir=''): kdeinstallinternal(lenv, files, subdir, 'KDEKCFG' )
	def kdesktopfile(lenv, files, subdir=''): kdeinstallinternal(lenv, files, subdir, 'KDEXDG' )
	...
	from SCons.Script.SConscript import SConsEnvironment
	SConsEnvironment.kdeInstallAuto = kdeInstallAuto
	SConsEnvironment.kdeheaders = kdeheaders
	SConsEnvironment.kdeappdata = kdeappdata
	SConsEnvironment.kdedata = kdedata
	SConsEnvironment.kdekcfg = kdekcfg
	SConsEnvironment.kdesktopfile = kdesktopfile
