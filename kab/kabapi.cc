...
KabAPI::ErrorCode KabAPI::add
(const AddressBook::Entry& entry,
 string& key,
 bool edit_)
{
  REQUIRE(widget!=0);
  ...
  if(dialog.exec())
    {
	 ...
	 "dialog finished with accept().\n");
      ...
      LG(GUARD, "KabAPI::add: dialog finished with "
	 "reject().\n");
      return Rejected;
    }  
...
KabAPI::ErrorCode KabAPI::getEntryByName
(const string&,
 list<AddressBook::Entry>&,
 const int)
{
