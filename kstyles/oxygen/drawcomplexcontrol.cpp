...
//                          pf | Tile::Center, false, QPoint(0,-uh));
   ...
   case CC_ComboBox: // A combobox, like QComboBox
      if (const QStyleOptionComboBox *cmb =
         ...
         QRect ar, r = RECT.adjusted(0,0,0,-dpi.f2);
         ...
         if (!ar.isNull()) {
            ...
            if (cmb->editable) {
               if (!sunken) {
                  painter->setPen(COLOR(Base).dark(105));
                  tmpOpt.rect =  ar.translated(-dpi.f2, dpi.f2);
...
#endif
               // for later (cosmetic)
               handle.translate(slider->upsideDown ? -dpi.f3 : dpi.f3, 0);
            }
         }
         ...
         if (slider->subControls & SC_SliderHandle) {
            ...
            fillWithMask(painter, xy,
                         Gradients::brush(btnBgColor(PAL, isEnabled, hover, step),
                                        dpi.SliderControl-dpi.f4, Qt::Vertical,
   ...
   case CC_TitleBar: // A Title bar, like what is used in Q3Workspace
          ...
          qstyleoption_cast<const QStyleOptionTitleBar *>(option))
      {
         ...
         if (option->subControls & SC_TitleBarLabel)
         {
            ...
            painter->drawText(ir.x() + dpi.f2, ir.y(), ir.width() - dpi.f2, ir.height(), Qt::AlignCenter | Qt::TextSingleLine, tb->text);
         }
