...
// TEMPORARY. In Qt5 this is QDir::removeRecursively
bool QTemporaryDir::removeRecursively(const QString& path)
{
    QFileInfo fi(path);
    if (!fi.exists())
        return true;
    ...
    while (di.hasNext()) {
        ...
        if (!ok) {
            qWarning("Failed to remove %s", qPrintable(di.filePath())); // not in qt5
            success = false;
        }
    }
