...
: KDialog(parent), d(new KPrintDialogPrivate)
{
	...
	QString whatsThisPrinterSelect = i18n(  " <qt><b>Printer Selection Menu:</b> "
						...
						" To reload the configuration files, either start kprinter again, or"
...
void KPrintDialog::setFlags(int f)
{
	...
	KConfigGroup group = cf->group("General");
	SHOWHIDE(d->m_default, ((f & KMUiManager::Default) && !group.readEntry("UseLast", true)))
...
void KPrintDialog::initialize(KPrinter *printer)
{
	...
	if (!plist.isEmpty())
	{
		...
		while (it.hasNext())
		{
			...
			d->m_printers->insertItem(SmallIcon(printerIt->pixmap(),0,(printerIt->isValid() ? (int)K3Icon::DefaultState : (int)K3Icon::LockOverlay)),printerIt->name(),false/*sep*/);
