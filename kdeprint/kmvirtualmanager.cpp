...
void KMVirtualManager::remove(KMPrinter *p, const QString& name)
{
    ...
    KMPrinter *printer = findPrinter(instname);
    if (!printer) return;
    if (name.isEmpty()) { // remove default instance => only remove options, keep the KMPrinter object
...
void KMVirtualManager::setAsDefault(KMPrinter *p, const QString& name, QWidget *parent)
{
    ...
    if (p->isSpecial()) {
        if (KMessageBox::warningContinueCancel(parent,
                                               ...
                                               QString(), KGuiItem(i18n("Set as Default")), KStandardGuiItem::cancel(), "setSpecialAsDefault") == KMessageBox::No)
            return;
    }

    KMPrinter *printer = findPrinter(instname);
    if (!printer) { // create it if necessary
        create(p, name);
...
void KMVirtualManager::refresh()
{
    ...
    } else { // parse printers looking for instances -> undiscarded them, real printers
...
void KMVirtualManager::loadFile(const QString& filename)
{
    ...
    if (f.exists() && f.open(QIODevice::ReadOnly)) {
        ...
        while (!t.atEnd()) {
            ...
            if (realprinter && !realprinter->isDiscarded()) { // keep only instances corresponding to an existing and
