...
void KMRlprManager::savePrintersConf(const QString& filename)
{
	...
	if (f.open(QIODevice::WriteOnly))
	{
      ...
      KMPrinter *printer(it.next());
			if (!printer->name().isEmpty() && printer->instanceName().isEmpty())
			{
				QString	host = printer->option("host");
				QString	queue = printer->option("queue");
				if (!host.isEmpty() && !queue.isEmpty())
				{
					t << printer->name() << '\t' << host << '\t' << queue;
					t << '\t' << printer->description() << '\t' << printer->location() << endl;
				}
			}
		}
	}
