...
#ifdef Q_WS_X11
    KWindowSystem::setIcons(winId(), *(m_icon->pixmap()), SmallIcon("document-print"));
#endif
...
{
	KNotification::event("printerror", i18n("<p><nobr>A print error occurred. Error message received from system:</nobr></p><br />%1", msg) ,  QPixmap(), 0L, KNotification::DefaultEvent);
    m_processpool.removeAll(proc);
}
...
QString KDEPrintd::openPasswordDialog(const QString& user)
{
    ...
    KPasswordDialog dlg(0L, KPasswordDialog::ShowUsernameLine);
