...
public:
    ...
    bool atEnd() const {
        return (m_stream.atEnd() && m_linebuf.isEmpty());
    }
    QString readLine();
    void unreadLine(const QString& l) {
        m_linebuf = l;
    }
private:
...
QString getPrintcapFileName()
{
    ...
    if (f.exists() && f.open(QIODevice::ReadOnly)) {
        kDebug() << "/etc/lpd.conf found: probably LPRng system";
        QTextStream t(&f);
        QString  line;
        while (!t.atEnd()) {
            line = t.readLine().trimmed();
            if (line.startsWith("printcap_path=")) {
                kDebug() << "printcap_path entry found: " << line;
                QString pcentry = line.mid(14).trimmed();
                kDebug() << "printcap_path value: " << pcentry;
                if (pcentry[0] == '|') { // printcap through pipe
                    ...
                    kDebug() << "printcap obtained through pipe" << endl << "executing: " << cmd;
    ...
    kDebug() << "printcap file returned: " << printcap;
    return printcap;
}
...
QString getEtcPrintersConfName()
{
    ...
    if (!QFile::exists(printersconf) && !KStandardDirs::findExe("ypcat").isEmpty()) {
        ...
        kDebug() << "printers.conf obtained from NIS server: " << cmd;
...
KMLpdUnixManager::KMLpdUnixManager(QObject *parent, const QStringList & /*args*/)
        : KMManager(parent)
{
    m_loaded = false;
}
