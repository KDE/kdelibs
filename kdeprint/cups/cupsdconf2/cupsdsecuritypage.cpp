...
void CupsdSecurityPage::slotAdd()
{
	...
	if (LocationDialog::newLocation(loc, this, conf_))
	{
		...
		for (locs_.first(); locs_.current(); locs_.next())
			...
			{
				if (KMessageBox::warningContinueCancel(this, i18n("This location is already defined. Do you want to replace the existing one?"),"",i18n("Replace")) == KMessageBox::Continue)
				{
					index = locs_.at();
					locs_.remove();
					break;
