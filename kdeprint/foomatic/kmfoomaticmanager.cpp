...
DrMain* KMFoomaticManager::loadPrinterDriver(KMPrinter *printer, bool)
{
	...
	{
		setErrorMsg(i18n("Some printer informations are missing"));
		return NULL;
	}

	QString	cmd = QString::fromLatin1("foomatic-combo-xml -p %1 -d %2").arg(printer->option("printer")).arg(printer->option("driver"));
