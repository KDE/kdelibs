...
public:
    ...
    QPaintEngine * paintEngine() const {
        return m_kPrinter->d->m_printer->paintEngine();
    }
    ...
    void updateState(const QPaintEngineState &state) {
        paintEngine()->updateState(state);
    }

    void drawRects(const QRect *rects, int rectCount) {
        paintEngine()->drawRects(rects, rectCount);
    }
    ...
    void drawLines(const QLine *lines, int lineCount) {
        paintEngine()->drawLines(lines, lineCount);
    }
    ...
    void drawPoints(const QPointF *points, int pointCount) {
        paintEngine()->drawPoints(points, pointCount);
    }
    ...
    void drawPolygon(const QPointF *points, int pointCount, PolygonDrawMode mode) {
        paintEngine()->drawPolygon(points, pointCount, mode);
    }
    ...
    void drawPixmap(const QRectF &r, const QPixmap &pm, const QRectF &sr) {
        paintEngine()->drawPixmap(r, pm, sr);
    }
    void drawTextItem(const QPointF &p, const QTextItem &textItem) {
        paintEngine()->drawTextItem(p, textItem);
    }
    void drawTiledPixmap(const QRectF &r, const QPixmap &pixmap, const QPointF &s) {
        paintEngine()->drawTiledPixmap(r, pixmap, s);
    }
    void drawImage(const QRectF &r, const QImage &pm, const QRectF &sr,
                   Qt::ImageConversionFlags flags = Qt::AutoColor) {
        paintEngine()->drawImage(r, pm, sr, flags);
    }

#ifdef Q_WS_WIN
    HDC getDC() const {
        return paintEngine()->getDC();
    }
    void releaseDC(HDC hdc) const {
        paintEngine()->releaseDC(hdc);
    }
#endif

    QPoint coordinateOffset() const {
        return paintEngine()->coordinateOffset();
    }

    Type type() const {
        return paintEngine()->type();
    }
...
QList<int> KPrinter::pageList() const
{
    ...
    if (mp > 0 && MP > 0 && MP >= mp) { // do something only if bounds specified
        if (option("kde-current") == "1") { // print only current page
            int pp = currentPage();
            ...
            } else { // add all pages between min and max
...
{
    kWarning(500) << "KPrinter::realPageSize() is obsolete, do not use";
    if (d->m_pagesize)
...
{
    if (p) {
        kDebug(500) << "Page size:  width =" << p->pageWidth();
        kDebug(500) << "Page size: height =" << p->pageHeight();
        kDebug(500) << "Page size:   left =" << p->leftMargin();
        kDebug(500) << "Page size:    top =" << p->topMargin();
        kDebug(500) << "Page size:  right =" << p->rightMargin();
        kDebug(500) << "Page size: bottom =" << p->bottomMargin();
    } else
        kDebug(500) << "Resetting page size";
...
{
    kWarning(500) << "KPrinter::setRealPageSize(QSize) is obsolete, do not use (no effect)";
}

// FIXME: remove for 4.0
void KPrinter::setRealDrawableArea(const QRect&)
{
    kWarning(500) << "KPrinter::setRealDrawableArea(QRect) is obsolete, do not use (no effect)";
}

// FIXME: remove for 4.0
QRect KPrinter::realDrawableArea() const
{
    kWarning(500) << "KPrinter::realDrawableArea() is obsolete, do not use";
    if (d->m_pagesize)
