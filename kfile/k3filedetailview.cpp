                                     ...
                                     const KFileItem *i )
{
    ...
    if ( spec & QDir::Time )
        item->setKey( sortingKey( i->time( KIO::UDSEntry::UDS_MODIFICATION_TIME ),
                                  i->isDir(), spec ));
...
void K3FileDetailView::slotSortingChanged( int col )
{
    ...
    for ( ; kit != kend; ++kit ) {
        ...
        if ( sortSpec & QDir::Time ) {
            ...
            key = sortingKey( (unsigned long)item->time( KIO::UDSEntry::UDS_MODIFICATION_TIME ), item->isDir(), sortSpec );
        }
