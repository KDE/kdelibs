...
void K3FileIconView::setSorting( QDir::SortFlags spec )
{
    ...
    for ( ; kit != kend; ++kit ) {
        ...
        if ( spec & QDir::Time ) {
            ...
            key = sortingKey( (unsigned long)item->time( KIO::UDSEntry::UDS_MODIFICATION_TIME ), item->isDir(), spec );
        }
                              ...
                              bool updateTextAndPixmap )
{
    ...
    if ( spec & QDir::Time )
        // warning, time_t is often signed -> cast it
        item->setKey( sortingKey( (unsigned long) i->time( KIO::UDSEntry::UDS_MODIFICATION_TIME ),
                                  i->isDir(), spec ));
    else if ( spec & QDir::Size )
