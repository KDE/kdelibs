...
class Konto::OntologyManager::Private
{
public:
...
Konto::OntologyManager* Konto::OntologyManager::instance()
{
...
Konto::OntologyManager::OntologyManager()
    : d( new Private( this ) )
{
}
...
void Konto::OntologyManager::setOntologyLoader( OntologyLoader* loader )
{
...
const Konto::Ontology* Konto::OntologyManager::getOntology( const QUrl& uri )
{
    ...
    //
        ...
        if ( !sl.isEmpty() ) {
            ...
            //
            qDebug() << "(Konto::OntologyManager) Could not find ontology " << uri << endl;
            return 0;
        }
    }
}
...
Konto::Ontology* Konto::OntologyManager::constructOntology( const QUrl& ontoUri, const QList<Statement>& sl )
{
    Ontology& ont = d->ontoCache[ ontoUri ];
    ont.d->uri = ontoUri;

    QString rdfsComment = QString( "%1%2" ).arg( rdfsNamespace() ).arg( "comment" );
    QString rdfsLabel = QString( "%1%2" ).arg( rdfsNamespace() ).arg( "label" );
    ...
    for ( QList<Statement>::const_iterator it = sl.constBegin(); it != sl.constEnd(); ++it ) {
        ...
        if( s.predicate().uri().toString().endsWith( "#subPropertyOf" ) ) {
            ...
            if ( s.object().toString().startsWith( xsdNamespace() ) ) {
        ...
        else if( s.predicate().toString() == rdfsComment ) {
        ...
        // load l10n'ed labels
        else if( s.predicate().toString() == rdfsLabel ) {
            if ( s.object().language().isEmpty() ) {
