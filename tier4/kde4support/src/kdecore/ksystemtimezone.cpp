...
{
    //qDebug() << "KSystemTimeZones::configChanged()";
...
void KSystemTimeZones::zonetabChanged(const QString &zonetab)
{
    Q_UNUSED(zonetab)
#ifndef Q_OS_WIN
    //qDebug() << "KSystemTimeZones::zonetabChanged()";
    KSystemTimeZonesPrivate::m_ktimezonedError = false;
...
void KSystemTimeZonesPrivate::readConfig(bool init)
{
    ...
    if (!group.exists()) {
        qCritical() << "No time zone information obtained from ktimezoned";
