    ...
    : QWidget( parent ), d(new Private)
{
    ...
    m_playButton->setIcon( KIcon( "player_play" ) );
...
void KNotifyWidget::updateWidgets( ListViewItem *item )
{
    ...
    m_soundPath->setUrl( KUrl( event.soundfile ) );
    ...
    // logfile settings
    m_logfilePath->setUrl( KUrl(event.logfile) );
    ...
    m_executePath->setUrl( KUrl(event.commandline) );
...
//    QString comment = QLatin1String("Comment");
    ...
    while ( it != conflist.end() ) {
        if ( (*it) != global && (*it) != default_group ) { // event group
            ...
            e->dontShow = kc->readEntry( "nopresentation", 0 );
            e->configGroup = *it;
            ...
            if ( e->name.isEmpty() || e->description.isEmpty() )
                ...
                int default_rep = kc->readEntry("default_presentation",
                                                   int(KNotifyClient::PassivePopup));
