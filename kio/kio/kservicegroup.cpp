...
KServiceGroup::entries(bool sort, bool excludeNoDisplay, bool allowSeparators, bool sortByGenericName)
{
    ...
    KSortableValueList<SPtr,Q3CString> slist;
    KSortableValueList<SPtr,Q3CString> glist;
	...
	bool noDisplay = p->isType(KST_KServiceGroup) ?
        ...
        KSortableValueList<SPtr,Q3CString> & list = p->isType(KST_KServiceGroup) ? glist : slist;
        QString name;
    ...
    for (QStringList::ConstIterator it(d->sortOrder.begin()); it != d->sortOrder.end(); ++it)
    {
        ...
        if (item[0] == '/')
        {
          ...
          for(KSortableValueList<SPtr,Q3CString>::Iterator it2 = glist.begin(); it2 != glist.end(); ++it2)
          {
        ...
        else if (item[0] != ':')
        {
          ...
          for(KSortableValueList<SPtr,Q3CString>::Iterator it2 = slist.begin(); it2 != slist.end(); ++it2)
          {
    ...
    for (QStringList::ConstIterator it(d->sortOrder.begin()); it != d->sortOrder.end(); ++it)
    {
        ...
        if (item[0] == ':')
        {
          ...
          else if (item == ":A")
          {
            ...
            KSortableValueList<SPtr,Q3CString>::Iterator it_s = slist.begin();
            KSortableValueList<SPtr,Q3CString>::Iterator it_g = glist.begin();
