...
public:
   ...
   * or a full pathname.
   */
  QString decode(const Q3CString& name) const;
   ...
   * or a full pathname.
   */
  Q3CString encode(const QString& name) const;

  /**
   * Converts the given URL into its 8-bit components
   */
  Q3CString encode(const KURL& url) const;
   ...
   */
  Q3CString directory(const KURL& url, bool ignore_trailing_slash = true) const;

  /**
   * Converts the given URL into 8-bit form and retrieve the filename.
   */
  Q3CString fileName(const KURL& url) const;
