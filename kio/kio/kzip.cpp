...
KZip::KZip( const QString& filename, const QString & _mimetype )
    : KArchive( 0L )
{
    kdDebug(7122) << "KZip(filename, _mimetype) reached." << endl;
    m_filename = filename;
    ...
    if ( mimetype.isEmpty() )
    {
        ...
        kdDebug() << "KZip::KZip mimetype=" << mimetype << endl;
...
KZip::KZip( QIODevice * dev )
    : KArchive( dev )
{
    kdDebug(7122) << "KZip::KZip( QIODevice * dev) reached." << endl;
    d = new KZipPrivate;
}
...
KZip::~KZip()
{
    ...
    kdDebug(7122) << "~KZip reached." << endl;
    if( isOpened() )
        close();
