    ...
    : d(new KOpenSSLProxyPrivate())
{
   ...
   if (!libname.isNull())
         d->cryptoLib = ll->globalLibrary(libname.toLatin1());
   }
...
// FIXME: #define here for the various OS types to optimize
   ...
   for (QStringList::Iterator it = libpaths.begin();
	 ...
	 if (!access(tmpStr.toLatin1(), R_OK))
            d->cryptoLib = ll->globalLibrary(alib.toLatin1());
   ...
   if (!libname.isNull())
         d->sslLib = ll->globalLibrary(libname.toLatin1());
   }
#else
   for (QStringList::Iterator it = libpaths.begin();
                              it != libpaths.end();
	 ...
	 tmpStr.replace(QRegExp("\\(.*\\)"), "");
	 if (!access(tmpStr.toLatin1(), R_OK))
         	d->sslLib = ll->globalLibrary(alib.toLatin1());
         if (d->sslLib) break;
      }
      if (d->sslLib) break;
   }
