...
public: // KDoc seems to barf on those typedefs and generates no docs after them
   ...
   * entries for example.
   * @param _group_or_size Group or size requested for the pixmap, see KIconLoader
   * @param _path Output parameter to get the full path. Seldom needed.
   */
  virtual QPixmap pixmap( int _group_or_size, QString * _path = 0L ) const;
  /**
   ...
   * @param _group_or_size Group or size requested for the pixmap, see KIcoLoader
   * @param _path Output parameter to get the full path. Seldom needed.
   */
  virtual QPixmap pixmap( const KURL& _url, int _group_or_size, QString * _path = 0L ) const;
  ...
  static QPixmap pixmapForURL( const KURL & _url, mode_t _mode = 0,
                               int _group_or_size = KIcon::SizeSmall, QString * _path = 0L );
...
public:
  ...
  virtual QString icon( const QString& _url, bool _is_local ) const;
  virtual QString icon( const KURL& _url, bool _is_local ) const;
  virtual QPixmap pixmap( const KURL& _url, int group_or_size, QString * _path = 0L ) const;
  virtual QString comment( const QString& _url, bool _is_local ) const;
  virtual QString comment( const KURL& _url, bool _is_local ) const;
