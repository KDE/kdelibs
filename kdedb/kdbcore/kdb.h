 ...
 * The implementation of the abstraction layer is done in DBMS specific
 * 'plugins', that are dynamically loadable libraries, loaded at runtime by
 * the engine itself.
 *
 * Through a plugin is possible to open connections to different database
 * servers.  Each Connection in turn provide access to the different
 * databases present in the server.
 *
 * A Database is a collection of Tables and stored Queries, and can be used
 * to open recordset based on entered queries (not stored ones).
 *
 ...
 *
 * Each DBMS implements its own type system. Most (but not all) of these
 * type systems are a superset of the SQL92 specification. The goal of the
 * KDB core library is to give the programmer a unified way to handle all
 * that diversity transparently.
 *
 * KDB defines a set of supported C++/QT types (see @ref KDB::DataType enum)
 * that are mapped at plugin level with the native types of the DBMS.  Those
 * types that does not fit naturally with the native C++ types are converted
 * either to QString or to QByteArray. This way, all the power of the
 * underlying DBMS can be used by an expert programmer, but the novice (and
 ...
 * support for these.
 *
 * For example: the @ref KDB::Field object implements all the conversion
 * operators for the standard types. The implementation for a specific field
 * type will throw an exception if an incorrect conversion is attempted.
 *
 ...
 *
 * Each Plugin object can be queried to know if a specific capability is
 * implemented or to obtain a list of implemented capabilities. A list of
 * currently supported capabilities is available in the enum @ref
 * KDB::capability.
 *
 ...
 *
 * The first kind of capability is implemented within the interface of the
 * base object that can implement it (like @ref
 * KDB::Connection::beginTransaction). This implementation will throw an
 * exception if the plugin does not support such a capability
 *
 * The second type of capability is implemented by the Plugin object,
 * through the @ref KDB::Plugin::createObject method. With this method the
 * programmer can obtain a KDB::Capability subclass that can be casted to
 * the specific type
 *
 * @sect Other useful notes
 *
 * Memory is managed internally by the library itself. Every object is
 * created within the library and either is destroyed when the library is
 * unloaded or is a reference counted object, that is unloaded when the last
