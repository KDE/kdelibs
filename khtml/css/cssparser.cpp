...
//     case CSS_PROP_PAGE:                 // <identifier> | auto // ### CHECK
	...
	valid_primitive = !strict && validUnit( value, FPercent|FNumber|FLength, strict&(!nonCSSHint) );
	break;
...
// #endif
	...
	if ( id != 0 )
	    ...
	    qDebug("primitive value is %f (unit=%d)", value->fValue, value->unit );
...
bool CSSParser::parseContent( int propId, bool important )
{
    ...
    while (curP < endP) {
        ...
        else if (str.length() && (str[0] == '\'' || str[0] == '"'))
        {
			...
			QString hex;
			bool ok;
			while ( i-initial<6 && i<l-1 && isHexadecimal( nextChar ) ) {
			    hex += nextChar;
			    i++;
			    nextChar = str[i+1];
		        }
