                                             ...
                                             int l, int r, int& w, int& ml, int& mr, int& x)
{
    ...
    else
    {
        ...
        //1. solve left & width.
        if (l==AUTO && w==AUTO && r!=AUTO) {
            w = qMin(kMax(m_minWidth - pab, cw - (r + ml + mr + pab)), m_maxWidth - pab);
            l = cw - ( r + w + ml + mr + pab);
        }
        else
        ...
        //3. solve width & right.
        if (l!=AUTO && w==AUTO && r==AUTO) {
            w = qMin(kMax(m_minWidth - pab, cw - (l + ml + mr + pab)), m_maxWidth - pab);
            r = cw - ( l + w + ml + mr + pab);
        }
        else
...
void RenderBox::calcAbsoluteVertical()
{
    ...
    if (hl.isFixed()) {
        if (cb->style()->boxSizing() == BORDER_BOX)
            ch = qMax(0, hl.value() - cb->borderTop() - cb->borderBottom());
        else
