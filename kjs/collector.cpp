...
bool Collector::collect()
{
  ...
  for (int block = 0; block < heap.usedBlocks; block++) {
	...
	goto skip_block_mark;
      ...
      unsigned short int flags = imp->_flags;
      if (!(flags & ValueImp::VI_DESTRUCTED)) {
	if ((flags & (ValueImp::VI_CREATED|ValueImp::VI_MARKED)) == ValueImp::VI_CREATED &&
	    ((flags & ValueImp::VI_GCALLOWED) == 0 || imp->refcount != 0)) {
	  imp->mark();
	}
      } else {
