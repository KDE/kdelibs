...
//		kDebug(282)<<"Set forced fixed width for widget 0 :"<<w;
	}
        else
	{
...
//		kDebug(282)<<"Set forced fixed width for widget 1 :"<<w;
	}
	setupMinMaxSize();
...
// //		kDebug(282)<<"Set forced fixed width for widget 0 :"<<h;
	}
        else
	{
...
//		kDebug(282)<<"Set forced fixed height for widget 1 :"<<h;
	}
	setupMinMaxSize();
    if (divider) divider->hide();
}
...
//  kDebug(282)<<"ResizeEvent :"<< ((initialised) ? "initialised":"not initialised")<<", "<< ((ev) ? "real event":"")<<", "<<(isVisible() ?"visible":"");
  ...
  if (initialised) {
    ...
    if (ev && isVisible() && divider->isVisible()) {
      // real resize event.
//       kDebug(282)<<"mKeepSize : "<< ((m_orientation == Horizontal) ? "Horizontal":"Vertical");
...
//      else kDebug(282)<<"Something else happened";
    }

/*
    // --- debugging information ---
    kDebug(282) << "isVisible() is         : " << isVisible();
    kDebug(282) << "Orientation            : " << (m_orientation==Horizontal?"Horizontal":"Vertical")
                                                << endl;
    kDebug(282) << "Splitter visibility    : " << divider->isVisible();;
    kDebug(282) << "Splitter procentual pos: " << xpos;
    if (c0->getWidget()) {
        dc=dynamic_cast<K3DockContainer*>(c0->getWidget());
        kDebug(282) << "Child 0 K3DockContainer?: " << dc;
    }
    if (c1->getWidget()) {
        dc=dynamic_cast<K3DockContainer*>(c1->getWidget());
        kDebug(282) << "Child 1 K3DockContainer?: " << dc;
    }
    kDebug(282) << "Child0                 : " << child0;
    kDebug(282) << "child1                 : " << child1;
*/
