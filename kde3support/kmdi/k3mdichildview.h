...
//
//----------------------------------------------------------------------------
#ifndef _K3MDI_CHILD_VIEW_H_
#define _K3MDI_CHILD_VIEW_H_
...
class K3MdiChildViewPrivate;

/**
  * @short Base class for all your special view windows.
  *
  * Base class for all MDI view widgets. K3Mdi stores additional information in this class
  * to handle the attach/detach mechanism and such things.
  *
  * All such windows 'lives' attached to a K3MdiChildFrm widget
  * managed by K3MdiChildArea, or detached (managed by the window manager.)
  * So remember that the K3MdiChildView::parent pointer may change, and may be 0L, too.
  *
  * There are 2 possibilities for you to put your widgets under MDI control:
  *
  * Either you inherit all the views from K3MdiChildView:
  *   \code
  *   class MyMdiWidget : public K3MdiChildView
  *   { .... };
  *   ...
  *   MyMdiWidget w;
  ...
  *   \endcode
  *
  * or you wrap them by a K3MdiChildView somehow like this:
  *
  * \code
  * void DocViewMan::addK3MdiFrame(QWidget* pNewView, bool bShow, const QPixmap& icon)
  * {
  *   // cover it by a K3Mdi childview and add that MDI system
  *   K3MdiChildView* pMDICover = new K3MdiChildView( pNewView->caption());
  ...
  *   // correct the default settings of K3Mdi ('cause we haven't a tab order for subwidget focuses)
...
protected:
	/**
	 * See K3MdiChildView::caption
	 */
	QString m_szCaption;
	
	/**
	 * See K3MdiChildView::tabCaption
	 */
	QString m_sTabCaption;
	
	/**
	 * See K3MdiChildView::focusedChildWidget
	 */
	QWidget* m_focusedChildWidget;
	
	/**
	 * See K3MdiChildView::setFirstFocusableChildWidget
	 */
	QWidget* m_firstFocusableChildWidget;
	
	/**
	 * See K3MdiChildView::setLastFocusableChildWidget
	 */
...
private:
	/**
	 * Internally used as indicator whether this K3MdiChildView is treated as document view or as tool view.
	 */
	bool m_bToolView;
...
public:
	 ...
	 * Constructor
	 */
	K3MdiChildView( const QString& caption, QWidget* parentWidget = 0L, const char* name = 0L, Qt::WFlags f = 0 );
	
	/**
	 * Constructor 
	 * sets "Unnamed" as default caption
	 */
	K3MdiChildView( QWidget* parentWidget = 0L, const char* name = 0L, Qt::WFlags f = 0 );
	...
	/**
	 * Returns the K3MdiChildFrm parent widget (or 0 if the window is not attached)
	 */
	K3MdiChildFrm *mdiParent() const;
...
public Q_SLOTS:
	...
	* to this K3MdiMainFrm you'll be faster calling that function.
	*/
	virtual void attach();
	
	/**
	 * Detaches this window from the Mdi manager.
	 * It calls the K3MdiMainFrm detachWindow function , so if you have a pointer
	 * to this K3MdiMainFrm you'll be faster calling that function.
	 */
	virtual void detach();
	 ...
	 * move to the main menubar (if set by K3MdiMainFrm::setMenuForSDIModeSysButtons ).
	...
	/**
	 * Internally called, if K3MdiMainFrm::attach is called.
	 * Actually, only the caption of the covering childframe is set.
	 */
	virtual void youAreAttached( K3MdiChildFrm *lpC );
	
	/**
	 * Internally called, if K3MdiMainFrm::detach is called.
	...
	/**
	 * Overridden from its base class method. Emits a signal K3MdiChildView::isMinimizedNow , additionally.
	 * Note that this method is not used by an external windows manager call on system minimizing.
	 */
	virtual void showMinimized();
	
	/**
	 * Overridden from its base class method. Emits a signal K3MdiChildView::isMaximizedNow , additionally.
	 * Note that this method is not used by an external windows manager call on system maximizing.
	 */
	virtual void showMaximized();
	
	/**
	 * Overridden from its base class method. Emits a signal K3MdiChildView::isRestoredNow , additionally.
...
protected:
	/**
	 * Ignores the event and calls K3MdiMainFrm::childWindowCloseRequest instead. This is because the
	 ...
	 * See K3MdiChildView::setFirstFocusableChildWidget and K3MdiChildView::lastFirstFocusableChildWidget
	 */
...
Q_SIGNALS:
	 ...
	 */
	void attachWindow( K3MdiChildView*, bool );
	
	/**
	 * Internally used by K3MdiChildView::detach to send it as command to the mainframe.
	 */
	void detachWindow( K3MdiChildView*, bool );
	 ...
	 * See K3MdiChildView::focusInEvent
	 */
	void focusInEventOccurs( K3MdiChildView* );
	
	/**
	 * Is sent when this MDI child has received the focus (after actually changing the focus).
	 * See K3MdiChildView::focusInEvent
	 */
	void gotFocus( K3MdiChildView* );
	
	/**
	 * Is sent when this MDI child was set to the activate view of all MDI views (after actually changing the focus).
	 * See K3MdiChildView::activate
	 */
	void activated( K3MdiChildView* );
	
	/** Is sent when this MDI child view has lost the focus (after actually changing the focus).
	 *  See K3MdiChildView::focusOutEvent
	 */
	void lostFocus( K3MdiChildView* );
	
	/** Is sent when this MDI child view was deactivated (after actually changing the focus).
	 *  See K3MdiChildView::focusOutEvent
	 */
	void deactivated( K3MdiChildView* );
	
	/**
	 * Internally used to send information to the mainframe that this MDI child view wants to be closed.
	 * See K3MdiChildView::closeEvent and K3MdiMainFrm::closeWindow
	 */
	void childWindowCloseRequest( K3MdiChildView* );
	
	/**
	 * Emitted when the window caption is changed via K3MdiChildView::setCaption or K3MdiChildView::setMDICaption
	 */
	void windowCaptionChanged( const QString& );
	
	/**
	 * Emitted  when the window caption is changed via K3MdiChildView::setTabCaption or K3MdiChildView::setMDICaption
	 */
...
private:
	K3MdiChildViewPrivate *d;
	QWidget *m_trackChanges;
};

inline K3MdiChildFrm *K3MdiChildView::mdiParent() const
{
	QWidget * pw = parentWidget();
	if ( pw != 0L )
		if ( pw->inherits( "K3MdiChildFrm" ) )
			return ( K3MdiChildFrm * ) pw;
	return 0L;
}

#endif //_K3MDICHILDVIEW_H_
