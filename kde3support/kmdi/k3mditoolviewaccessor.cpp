...
K3MdiToolViewAccessor::K3MdiToolViewAccessor( K3MdiMainFrm *parent, QWidget *widgetToWrap, const QString& tabToolTip, const QString& tabCaption )
		: QObject( parent )
{
	mdiMainFrm = parent;
	d = new K3MdiToolViewAccessorPrivate();
	if ( widgetToWrap->inherits( "K3DockWidget" ) )
	{
		d->widgetContainer = dynamic_cast<K3DockWidget*>( widgetToWrap );
...
K3MdiToolViewAccessor::K3MdiToolViewAccessor( K3MdiMainFrm *parent )
{
	mdiMainFrm = parent;
	d = new K3MdiToolViewAccessorPrivate();
}

K3MdiToolViewAccessor::~K3MdiToolViewAccessor()
{
...
QWidget *K3MdiToolViewAccessor::wrapperWidget()
{
	if ( !d->widgetContainer )
	{
		d->widgetContainer = mdiMainFrm->createDockWidget( "K3MdiToolViewAccessor::null", QPixmap() );
		connect( d->widgetContainer, SIGNAL( widgetSet( QWidget* ) ), this, SLOT( setWidgetToWrap( QWidget* ) ) );
	}
	return d->widgetContainer;
}

QWidget *K3MdiToolViewAccessor::wrappedWidget()
{
	return d->widget;
}


void K3MdiToolViewAccessor::setWidgetToWrap( QWidget *widgetToWrap, const QString& tabToolTip, const QString& tabCaption )
{
	Q_ASSERT( !( d->widget ) );
	Q_ASSERT( !widgetToWrap->inherits( "K3DockWidget" ) );
	disconnect( d->widgetContainer, SIGNAL( widgetSet( QWidget* ) ), this, SLOT( setWidgetToWrap( QWidget* ) ) );
	delete d->widget;
	d->widget = widgetToWrap;
	K3DockWidget *tmp = d->widgetContainer;
...
bool K3MdiToolViewAccessor::eventFilter( QObject *, QEvent *e )
{
...
void K3MdiToolViewAccessor::placeAndShow( K3DockWidget::DockPosition pos, QWidget* pTargetWnd , int percent )
{
	place( pos, pTargetWnd, percent );
	show();
}
void K3MdiToolViewAccessor::place( K3DockWidget::DockPosition pos, QWidget* pTargetWnd , int percent )
{
	...
	{
		d->widgetContainer->setEnableDocking( K3DockWidget::DockNone );
	...
	{   // add (and dock) the toolview as DockWidget view
		...
		K3DockWidget* pCover = d->widgetContainer;

		K3DockWidget* pTargetDock = 0L;
		if ( pTargetWnd->inherits( "K3DockWidget" ) || pTargetWnd->inherits( "K3DockWidget_Compat::K3DockWidget" ) )
		{
			pTargetDock = ( K3DockWidget* ) pTargetWnd;
		}
		...
		if ( !pTargetDock || pTargetWnd == mdiMainFrm->getMainDockWidget() )
		{
			...
			{
				K3DockWidget * dw1 = pTargetDock->findNearestDockWidget( pos );
				if ( dw1 )
					pCover->manualDock( dw1, K3DockWidget::DockCenter, percent );
				else
...
void K3MdiToolViewAccessor::hide()
{
	Q_ASSERT( d->widgetContainer );
	if ( !d->widgetContainer )
		return ;
...
void K3MdiToolViewAccessor::show()
{
	Q_ASSERT( d->widgetContainer );
	if ( !d->widgetContainer )
		return ;
...
#include "k3mditoolviewaccessor.moc"
#endif 
