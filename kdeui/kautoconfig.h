...
/**
 * The base class for widgets that KAutoConfig can parse.
 * 
 * Implimenting this class for a widget and then using
 * addKAutoConfigWidget(...) in KAutoConfig one can enable widgets
 * that are not supported by default in KAutoConfig.
 * 
 * See kautoconfigwidgets.h for the default KAutoConfig widgets.
 */ 
class KAutoConfigWidget {

public:
  /**
   * Constructor
   * @param kconfig - The KConfig that should be used when retrieving/saving
   * @param widget - The widget that the class performs actions on.
   */
  KAutoConfigWidget( KConfig *kconfig=NULL, QWidget *widget=NULL);
  virtual ~KAutoConfigWidget() {};
  
  /**
   * Stores the current value as default and then reads the value in
   * from the config
   */ 
  virtual void retrieveSettings() = 0;

  /**
   * If the setting is not the same as the default or what the KConfig
   * currently has then the new value is stored.
   * @return true if a new value is stored.
   */ 
  virtual bool saveSettings() const = 0;

  /**
   * Sets the widget back to the default value.
   * @return true if the setting is not the default and was changed.
   */ 
  virtual bool resetSettings() const = 0;

  /**
   * Helper function for KAutoConfig which simply returns a new copy
   * of itself with widget and config already set.
   */ 
  virtual KAutoConfigWidget* makeKAutoConfigWidget(QWidget *widget) const = 0;
 
  /**
   * Helper function for KAutoConfig to set the config for KAutoConfigWidget's
   * that are passed to KAutoConfig
   */ 
  inline void setConfig(KConfig *kconfig){ config = kconfig; };
 
protected:
  // Name of the widget (widget->name())
  QString widgetName;
  // KConfig to save settings of this widget to.
  KConfig *config;
 ...
 *
 * At this time KAutoConfig supports by default:
 ...
 * to be ignored.  If there is a unsupported widget that is a child of one of
 * the passed in widgets that does saves/restore/reset outside of KAutoConfig
 * please have KAutoConfig ignore it because future version of KAutoConfig
 * could support that widget causing problems.  If possible contact @author
 * to have that widget added to this class.
 * 
...
public:
  ...
  KAutoConfig(QObject *parent=0, const char *name=0,
	      KConfig *kconfig = KGlobal::config(), bool loadAllKnown=true);	
  ...
  /**
   * Adds a widget to the list of known widgets that are supported by this
   * class.  KAutoConfig will maintain and delete object.
   * @param name - className() such as QSpinBox.  Not name(), but className().
   * @param object - the KAutoConfigWidget that can maintain the widget name
   */ 
  void addKAutoConfigWidget(const char* name, KAutoConfigWidget *object);
