  ...
  if (kmainwidget)
   {
     ...
     if(w->minimumSize() == w->maximumSize()) {
       ...
       if(si != size()) {
	 nores = TRUE;		// prevent indef. recursion
	 setFixedSize(si);
	 nores = FALSE;
	 updateRects();
	 reclvl--;
     ...
     if(w->minimumSize().width() > s.width())
       newsize.setWidth(w->minimumSize().width());
     else if(w->maximumSize().width() < s.width())
       newsize.setWidth(w->maximumSize().width());

     // check for vertical constraints
     if(w->minimumSize().height() > s.height())
       newsize.setHeight(w->minimumSize().height());
     else if(w->maximumSize().height() < s.height())
       newsize.setHeight(w->maximumSize().height());     
	 ...
	 si.setHeight(si.height() - s.height() + w->minimumSize().height());
	 newmins = si;
	 if(newmins.width() > s.width() || newmins.height() > s.height())
	   resize(newmins);
	 si = size();
	 si.setWidth(si.width() - s.width() + w->maximumSize().width());
	 si.setHeight(si.height() - s.height() + w->maximumSize().height());
	 newmaxs = si;
	 if(newmaxs.width() < s.width() || newmaxs.height() < s.height())
	   resize(newmaxs);
	 QTimer::singleShot(0, this, SLOT(setFixed()));
	 nores = FALSE;
	 updateRects();
	 reclvl--;
	 return;
     ...
     // remember old mimumum and maximum sizes
     QSize si;
     QSize mins = w->minimumSize();
     QSize maxs = w->maximumSize();
     si = size();
     newmins.setWidth(si.width() - s.width() + mins.width());
     newmins.setHeight(si.height() - s.height() + mins.height());
     newmaxs.setWidth(si.width() - s.width() + maxs.width());
     newmaxs.setHeight(si.height() - s.height() + maxs.height());  
...
void KTopLevelWidget::setFixed() {
  nores = TRUE;
  setMinimumSize(newmins);
  setMaximumSize(newmaxs);
  nores = FALSE;
}
