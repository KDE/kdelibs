...
KDialogBase::KDialogBase( QWidget *parent, const char *name, bool modal,
  ...
  :KDialog( parent, caption , (ButtonCode)buttonMask , Qt::MSWindowsFixedSizeDialogHint , user1, user2, user3 ),
...
KDialogBase::KDialogBase( const QString &caption, int buttonMask,
  ...
  :KDialog( parent, caption ,(ButtonCode) buttonMask , Qt::MSWindowsFixedSizeDialogHint ),
   mJanus(0) ,  mShowTile(false), d(new KDialogBasePrivate)
{
  setObjectName( name );
  setDefaultButton( defaultButton );
...
void KDialogBase::printMakeMainWidgetError()
{
  if( mJanus )
  {
    kWarning(240) << "KDialogBase::makeMainWidget: Illegal mode (wrong constructor)" << endl;
  }
}
