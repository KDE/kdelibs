...
WinCpu::WinCpu(WinDevice *device):
    WinInterface(device)
{
...
int WinCpu::number() const
{
    return m_number;
}

int WinCpu::maxSpeed() const
{
...
Solid::Processor::InstructionSets WinCpu::instructionSets() const
{
    static Solid::Processor::InstructionSets set = Solid::Backends::Shared::cpuFeatures();
    return set;
}

QStringList WinCpu::getUdis()
{
    QStringList out;
...
DWORD WinCpu::countSetBits(ULONG_PTR bitMask)
{
...
const QMap<int,WinCpu::ProcessorInfo> &WinCpu::updateCache()
{
